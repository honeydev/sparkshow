...
class ExampleTestSuite extends AnyFlatSpec with HttpApp with BeforeAndAfter {
  ...
  def testTransactor(config: DBConf) = {
    for {
  ...
  before {
    testTransactor(config.db).use((transactor: HikariTransactor[IO]) => {
      transactor.configure { ds => IO { 
        Flyway
          .configure()
          .dataSource(ds)
          .load()
  ...
  "test" should "sucdess" in {
    ...
    testTransactor(config.db).use((transactor: HikariTransactor[IO]) => {
      val httpApp = buildHttpApp(transactor)
      val res = httpApp.orNotFound.run(Request(method = Method.GET, uri = uri"/auth")).unsafeRunSync

      val response = res.as[String].unsafeRunSync
      IO(ExitCode.Success)
    }).unsafeRunSync()
    assert(true)
  }
